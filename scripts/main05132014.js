...
function hashRead() {
    ...
    if (window.location.hash.length > 1) {
        ...
        if (hash3 && hash3.length > 0 && parseInt(hash3,10) !== record.id) {
            if (hash3.indexOf(",") == -1) {
                if(activeLayer=="census"){
                  changeCensusBlock(hash3, true);
                  var layer = getCBLayer(hash3);
                  cbReportID = hash3;
...
function changeCensusBlock(nhid) {
...
function changeNeighborhood(nhid) {
