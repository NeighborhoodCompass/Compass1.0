...
function prettyCBMetric(x, metric) {
	console.log("the prettyCBMetric " + x);
    if (isNumber(x)) {
        if (x >=1000 && metric.substring(0, 6) != "RAVGYR") x = x.toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, ",");
        return (CBmeta[metric].style.prefix ? CBmeta[activeCBMeasure].style.prefix : "") + x + (CBmeta[metric].style.units ? CBmeta[activeCBMeasure].style.units : "");
    }
    else {
        return "N/A";
    }
}
function prettyNHMetric(x, metric) {
    console.log("prettyNHMetric " + x);
    if (isNumber(x)) {
        if (x.toString().indexOf(".")>1){x = x.toFixed(1);}
        if (x >=1000 && metric.substring(0, 6) != "RAVGYR") x = x.toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, ",");
        return (NHmeta[metric].style.prefix ? NHmeta[activeNHMeasure].style.prefix : "") + x + (NHmeta[metric].style.units ? NHmeta[activeNHMeasure].style.units : "");
    }
    else {
        return "N/A";
    }
...
function calcCBAverage(measure) {
    if (!CBmeta[activeCBMeasure].style.avg) {
        var theSum = 0;
        var theCount = 0;
        $.each(cbJSONData.features, function() {
            theSum = theSum + this.properties[measure];
            //TODO - we may need to pass a year variable here
            if (this.properties[measure] !== null) theCount++;
            //old time series
            //if (this.properties[measure+"_13"] !== null) theCount++;
            
            // theSum = theSum + this.properties[measure];
            // if (this.properties[measure] !== null) theCount++;
        });
        CBmeta[measure].style.avg = Math.round(theSum / theCount);
    }
}
// function calcCBAverage(measure) {
    // if (!CBmeta[activeCBMeasure].style.avg) {
        ...
        // var multiCount = 0;
        // $.each(cbJSONData.features, function() {
            // theValue = this.properties[measure];
            // theSum = theSum + theValue;
            // theID = this.properties["ID"];
//             
            // if(upperMulti.length <= 5)
            // {
            	// multiObject = [theValue,theID];
            	// upperMulti.push(multiObject);
            	// lowerMulti.push(multiObject);
            	// upperMulti.sort(function sortFunction(a, b) {
				    // if (a[0] === b[0]) {
				        // return 0;
				    // }
				    // else {
				    ...
				    // }
				// });
				// lowerMulti.sort(function sortFunction(a, b) {
				    // if (a[0] === b[0]) {
				        // return 0;
				    // }
				    // else {
            ...
            // }
            // else{
            	// for (var i = 0; i < upperMulti.length; i++){
            		// value = upperMulti[i][0];
            		// if (theValue >= value){
            			// upperMulti.splice((i),0, [theValue,theID]);
            			// upperMulti.pop(4);
            			// console.log("Last upperMulti = "+upperMulti.toString());
            			// break;
            		// }
            	// }
            	// for (var i = 0; i < lowerMulti.length; i++){
            		// value = lowerMulti[i][0];
            		// if (theValue <= value){
            			// lowerMulti.splice((i),0, [theValue,theID]);
            			// lowerMulti.pop(4);
            			// console.log("Last lowerMulti = "+lowerMulti.toString());
            			// break;
            		// }
            	// }
            // }
