...
class PDF extends FPDF
{
    function Footer()
    {
        ...
        $this->Cell(0,0,$this->Image('Compass_ReportFooterNoCo.png',0.35,10.33,0,0,'PNG', 'http://compass.durhamnc.gov/index.html').'Page '.$this->PageNo().'                 Durham Neighborhood Compass - Report generated on: ' . $today['year'] . "." . $today['mon'] . "." . $today['mday'],0,0,'R', false, 'http://compass.durhamnc.gov/index.html');
  }
	...
	************************************************************/
	...
	if (strlen($measures[0]) > 0) {
	    ...
	    for ($i=0; $i < ceil(count($measures) / $mpp); $i++) {
	        ...
	        if (!empty($measures[$measureCount])) createMeasure(0.5, 0.5, $theLayer, $measures[$measureCount],$theNPAData,$theOID);
	        if (!empty($measures[$measureCount + 1])) createMeasure(4.3, 0.5, $theLayer, $measures[$measureCount + 1],$theNPAData,$theOID);
	        if (!empty($measures[$measureCount + 2])) createMeasure(0.5, 5.5, $theLayer, $measures[$measureCount + 2],$theNPAData,$theOID);
	        if (!empty($measures[$measureCount + 3])) createMeasure(4.3, 5.5, $theLayer, $measures[$measureCount + 3],$theNPAData,$theOID);
...
// Variable Report
// ************************************************************/
function createMeasure($x, $y, $theLayer, $themeasure, $npadata, $theID) {
    ...
    $pdf->SetY($y);
    $pdf->SetX($x);
	$pdf->Link($x, $y, 3.5, 0.22, 'http://compass.durhamnc.gov/index.html#/'.$theLayer.'/'.$themeasure.'/'.$theID);//.$substrYear.'/'.$theLayer.'/'.$themeasure.'/'.$theID);
