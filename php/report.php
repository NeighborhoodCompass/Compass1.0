...
// Load data JSON
if($reportRadioValue == 'census') {
  $string = file_get_contents("../scripts/JohnTSmetricsBGsDEV.json");
  $json = json_decode($string, true);
    
} 
elseif($reportRadioValue == 'neighborhood') {
  $string = file_get_contents("../scripts/JohnTSmetricsNHsDEV.json");
  $json = json_decode($string, true);
}
...
if (count($measures) > 0) {
  if($reportRadioValue == 'census') {
      $string = file_get_contents("../scripts/TSDurhamBGs_ratios.json");
  ...
  elseif($reportRadioValue == 'neighborhood') {
      $string = file_get_contents("../scripts/TSDurhamNHs_ratios.json");
      $npa_json = json_decode($string, true); 
      foreach ($npa_json["features"] as $value) {
...
// Prety up the number
function prettyData($fdata, $themeasure) {
    global $pdf, $json, $npadata, $bgdata;
	$prefix = "";
	$suffix = "";
	if (array_key_exists($themeasure, $fdata)){
		$data = $fdata[$themeasure];
	    if (is_null($data) || !is_numeric($data)) {
	        return "N/A";
	    }
	    else {
	        if ($data >= 1000) $data = number_format($data);
...
function getCountyAverage($fdata, $themeasure){
	...
	if (array_key_exists($themeasure, $fdata)){
	    ...
	    $average = round($measureSum/$count,0);
		if ($average >= 1000) $average = number_format($average);
...
$pdf->Image('reportCoverPageCompass.png',0,0,8.5);
	...
	// $pdf->Ln(5);
	// $pdf->Cell(0.3);$pdf->Write(0.2,WordWrap("OIDs = ".implode(" ",$OIDs), 75));
...
foreach($features as $key =>$feature){
	...
	// $pdf->SetFont('Arial','B',10);
	// $pdf->Ln(5);
	// $pdf->Cell(0.3);$pdf->Write(0.2,WordWrap("theOID = ".implode(" ",$measures), 75));
...
// 	
	...
	if($reportRadioValue == 'census') {
		$testExtent = file_get_contents('http://172.16.30.251/arcgis/rest/services/SharedMaps/CompassBaseMap2/MapServer/1/query?where=ID='.$theOID.'&f=json'); 
	} elseif($reportRadioValue == 'neighborhood') {
		$testExtent = file_get_contents('http://172.16.30.251/arcgis/rest/services/SharedMaps/CompassBaseMap2/MapServer/2/query?where=ID='.$theOID.'&f=json'); 
	}
	...
	if($reportRadioValue == 'census') {
		$mapURL = "http://gisweb2.durhamnc.gov/arcgis/rest/services/SharedMaps/CompassBaseMap2/MapServer/export?bbox=" . implode(",", $myFinalExtent) . "&layerDefs=1:ID=" . $theOID . "&layers=hide:2&width=800&height=512&dpi=150&format=png&f=image";
	} elseif($reportRadioValue == 'neighborhood') {
		$mapURL = "http://gisweb2.durhamnc.gov/arcgis/rest/services/SharedMaps/CompassBaseMap2/MapServer/export?bbox=" . implode(",", $myFinalExtent) . "&layerDefs=2:ID=" . $theOID . "&layers=hide:1&width=800&height=512&dpi=150&format=png&f=image";
	}
...
// Variable Report
// ************************************************************/
function createMeasure($x, $y, $theLayer, $themeasure, $featuredata, $theFeature) {
	...
	// $pdf->SetFont('Arial','B',10);
	// $pdf->Ln(5);
	// $pdf->Cell(0.3);$pdf->Write(0.2,WordWrap("themeasure = ".$featuredata[$themeasure.$theDate], 75));
    // //this block wordwraps
	// $pdf->SetTextColor(0,0,45);
	// $pdf->SetFont('Arial','B',10);
	// $pdf->Ln(5);
	// $pdf->Cell(0.3);$pdf->Write(0.2,WordWrap("theOID = ".implode(" ",$featuredata), 75));
    
    
    $measureRoot = substr($themeasure, 0, -3);
    $thisMeasure = $measureRoot.$yearSuffix;
    
    // //value and title
    $pdf->SetTextColor(10,0,128);
    $pdf->SetY($y);
    $pdf->SetX($x);
	$pdf->Link($x, $y, 3.5, 0.22, 'http://compass.durhamnc.gov/#/'.$substrYear.'/'.$theLayer.'/'.$measureRoot.'/'.$theFeature);
    $pdf->SetFont('Arial','BU',12);
    $pdf->MultiCell(3.5, 0.22, utf8_decode($json[$thisMeasure]['title']), 0, "L");
    
    
    
    // $pdf->MultiCell(3.5, 0.15, $featuredata[$json[$themeasure]['field']] . " " . utf8_decode($json[$themeasure]['title']), 0, "L");
	$pdf->SetTextColor(0,0,0);
    $pdf->Ln(0.07);
	$pdf->SetX($x);
    $pdf->SetFont('Arial','B',10);
	// $keys = array('red', 'green', 'blue');
	// $values = array('car', 'bike', 'truck');
	// $finalArray = array();
	// $finalArray = array_combine($keys, $values);
	// $red = 'POP_14';
	//TODO - fix this
	$pdf->MultiCell(3.5, 0.15, $targetLayer." Value: ".prettyData($featuredata, $thisMeasure), 0, "L");
    // $pdf->MultiCell(3.5, 0.15, $targetLayer." Value: ".);//prettyData($featuredata[$themeasure.$yearSuffix], $themeasure), 0, "L");
	...
	$pdf->SetX($x);
    $pdf->SetFont('Arial','B',10);
    $pdf->MultiCell(3.5, 0.15, $featureUniverse." Average: ".getCountyAverage($featuredata, $thisMeasure), 0, "L");
    ...
    $pdf->SetX($x);
    $pdf->SetFont('Arial','',8);
    $pdf->MultiCell(3.5, 0.15, utf8_decode(strip_tags($json[$thisMeasure]['description'])), 0, "L");
    ...
    $pdf->SetX($x);
    $pdf->SetFont('Arial','',8);
    $pdf->MultiCell(3.5, 0.15, utf8_decode(strip_tags($json[$thisMeasure]['importance'])), 0, "L");
    ...
    $pdf->SetX($x);
    $pdf->SetFont('Arial','',8);
    $pdf->MultiCell(3.5, 0.15, utf8_decode(strip_tags($json[$thisMeasure]['tech_notes'])) . "\n\n". utf8_decode(strip_tags($json[$thisMeasure]['source'])), 0, "L");
}
